...
-- a la API REST (el mismo tipo de hueco que Seguridad ya había cazado para
-- los TOPES de recibi_aplicaciones, ver recibi_aplicaciones_trigger_topes.sql).
--
-- 1) justificante_path NOT NULL para tipo='recibi', NOT VALID (no rompe
--    filas viejas, sí exige el dato en cualquier INSERT/UPDATE nuevo).
-- 2) La regla "al menos una factura que salda" es cruzada entre tablas (la
--    fila del recibí en `facturas` + N filas en `recibi_aplicaciones`) y NO
--    se puede expresar como CHECK — un CHECK es por fila, no puede saber si
--    OTRA tabla tiene filas relacionadas. La solución es que las dos
--    inserciones dejen de ser dos peticiones REST sueltas (lo que hacía
--    facturas/index.html) y pasen a ser una sola función atómica: si
--    p_aplicaciones llega vacío, la función entera falla y no se crea NADA,
--    ni el recibí ni sus aplicaciones — no puede quedar un recibí huérfano.
...
declare
  ...
  v_justificante text;
begin
  if not (public.es_agente() and public.puede('facturas')) then -- puede(herramienta text), verificado en pg_proc
    raise exception 'sin permiso para crear recibís' using errcode = '42501';
  end if;

  v_contrato_id := nullif(p_factura->>'contrato_id', '')::uuid;
  v_justificante := nullif(p_factura->>'justificante_path', '');
  if v_contrato_id is null then
    raise exception 'el recibí necesita un contrato' using errcode = '23514';
  end if;
  ...
  if p_id is null then
    ...
    if v_id is null then
      raise exception 'no se pudo actualizar: no existe, está anulado, o no es tuyo' using errcode = '42501';
    end if;
