...
comment on function public.contrato_identificadores(jsonb) is
  'Pasaporte y email (adq1 + adicionales) en minusculas, para decidir si dos contratos son del mismo comprador. Mismo criterio que sincronizar_compradores.';
...
begin
  ...
  select c.id, c.numero, c.tipo into ocupada_id, ocupada, ocupada_tipo
  ...
  if ocupada is not null and traspaso_ok then
    ...
    select public.contrato_identificadores(c.datos) into ids_ocupa
      from public.contratos c where c.id = ocupada_id;
