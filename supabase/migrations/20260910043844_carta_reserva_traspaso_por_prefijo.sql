-- destructivo-ok: falso positivo del guardrail no_destruir.py — matchea el
-- literal tg_op = 'UPDATE' (comparacion de trigger, no una sentencia DML) sin
-- WHERE despues en ese trozo; las UPDATE reales de esta funcion sí llevan
-- WHERE, sin cambios respecto a la version ya viva en produccion.
--
...
-- que debería sucederla muere con "La parcela X ya esta asignada al contrato CP00002".
-- CAUSA: traspaso_ok miraba ocupada_tipo in ('carta_reserva','carta_reserva_ampliada',
-- 'carta_reserva_hak_sewa') -- lista a mano del 14-ago que no incluyó las dos
-- Cartas dadas de alta después (carta_reserva_pma 7-sep, carta_reserva_investor_deck 10-sep).
-- ARREGLO: se generaliza a like 'carta_reserva%', el prefijo que ya comparten
-- por diseño todas las Cartas de Reserva (ver contracts/app.html), en vez de
-- ampliar la lista una tercera vez. Ver contracts/sql/carta_reserva_traspaso_por_prefijo.sql
-- en el repo para el detalle completo.
