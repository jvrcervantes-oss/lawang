...
as $$
  ...
  union all
  select 'aviso', 'firma_sin_pdf', c.numero, c.id, c.comprador_nombre,
         'El contrato esta cerrado pero una de sus firmas no guardo su PDF (pdf_path vacio).',
         c.created_at
    from public.contratos c
   where coalesce(c.bloqueado, false)
     and exists (select 1 from public.contrato_firmas cf
                  where cf.contrato_id = c.id and cf.estado = 'firmado' and cf.pdf_path is null)
