      ...
      select m.slug,
             jsonb_strip_nulls(jsonb_build_object(
               ...
               'extras', (select jsonb_object_agg(e.clave, jsonb_build_object(
                                   'nombre', e.nombre, 'desc', e.descripcion, 'precio', me.precio))
...
comment on function public.catalogo_publico() is
  'Lo único del catálogo que ve un anónimo: modelos publicados, con specs, techos y extras (nombre+descripción+precio, 21-sep-2026 — antes solo el precio, la web duplicaba nombre/desc a mano en modelo/datos.php). Nunca notas, ni precios por proyecto (modelos_villa), ni nada de unidades. Declarada en departamentos/seguridad/rls_publico.txt.';
;
