-- destructivo-ok: sustituye deck_faq_guarda y retira el helper _deck_texto_prohibido (lo sustituye _deck_introduce_prohibido); no toca filas.
-- Frontera bloque 3 (27-sep-2026, LAW-336), autorevisión: el freno de Legal de las FAQ del deck comparaba el
-- PRIMER término prohibido del texto de antes con el de ahora. Si el texto viejo ya decía «nominee» y el nuevo
-- añadía «Hak Milik», el servidor lo dejaba publicar (solo la pantalla lo paraba). Ahora se mira término a
-- término, como hace la pantalla (LEG_PROHIBIDO de editores.js): se bloquea cualquier término que esté en el
-- texto nuevo y no estuviera en el de antes.
create or replace function public._deck_introduce_prohibido(p_antes text, p_ahora text) returns text
language sql immutable set search_path = '' as $$
  select t.etiqueta from (values
    (1, 'nominee', '«nominee»'),
    (2, 'hak\s*milik', '«Hak Milik» (Lawang no ofrece Hak Milik)'),
    (3, '(garantizad|guaranteed?|dijamin).{0,40}(rentab|retorno|return|yield|imbal|beneficio|profit)|(rentab|retorno|return|yield|imbal|beneficio|profit).{0,40}(garantizad|guaranteed?|dijamin)',
        'una rentabilidad garantizada')) as t(orden, patron, etiqueta)
   where coalesce(p_ahora, '') ~* t.patron and not (coalesce(p_antes, '') ~* t.patron)
   order by t.orden limit 1
$$;
revoke all on function public._deck_introduce_prohibido(text, text) from public, anon, authenticated;
...
drop function if exists public._deck_texto_prohibido(text);
